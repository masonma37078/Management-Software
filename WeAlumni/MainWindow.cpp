...
Void WeAlumni::MainWindow::Rec_btn_Search_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    String^ cmd = "SELECT Record.Id AS 'ID', Record.Time AS '鐧昏鏃堕棿'," +
        " Record.StfId AS '鍛樺伐缂栧彿'," + " Record.Memname AS '鍛樺伐濮撳悕'," +
        " Staff.Dept AS '鎵�鍦ㄩ儴闂?," + "Staff.Position AS '鑱屼綅鑱屽姟'," +
        " Record.Action AS '鎿嶄綔鍐呭'" + "FROM Record, Staff WHERE Record.MemId = Staff.MemId AND ";
    String^ cmd1 = "";
...
Void WeAlumni::MainWindow::OPT_btn_Search_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    String^ command = "SELECT OPT.Id                                                        AS 'OPT缂栧彿', " +
        "OPT.Status                                                    AS '鐘舵�?, " +
        "(SELECT Member.Name FROM Member WHERE Member.Id = OPT.MemId)  AS '鎴愬憳濮撳悕', " +
        "(SELECT Member.Name FROM Member " +
        "INNER JOIN Staff INNER JOIN OPT " +
        "WHERE Member.Id = Staff.MemId AND Staff.MemId = OPT.StfId)    AS '鍛樺伐濮撳悕', " +
        "OPT.StartDate                                                 AS '寮�濮嬫棩鏈?, " +
        "OPT.EndDate                                                   AS '缁撴潫鏃ユ湡', " +
        "OPT.Title                                                     AS '澶磋', " +
        "OPT.Position                                                  AS '鑱屼綅' " +
...
Void WeAlumni::MainWindow::min_btn_accept_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    try {
        ...
        min_lbl_warning->Text = e->Message;//"更新数据失败！";
        min_DisableChange();
        min_btn_changeInfo->Enabled = false;
        min_GeneralInformation();
        return;
