...
#define CREATE_LOG_TBL "CREATE TABLE IF NOT EXISTS Log ( Id INT PRIMARY KEY, Time TEXT, StfId INT, Action TEXT, FOREIGN KEY(StfId) REFERENCES Member(Id) );"
#define CREATE_ITEM_TBL "CREATE TABLE IF NOT EXISTS Item ( Id INT PRIMARY KEY, Status TEXT, Name TEXT, Comment TEXT, Price TEXT );"
#define CREATE_ORDERS_TBL "CREATE TABLE IF NOT EXISTS Orders( Id INT PRIMARY KEY, Status TEXT, Time TEXT, MemId INT, StfId INT, ItemId TEXT, Amount INT, Price TEXT, Comment TEXT, FOREIGN KEY(MemId) REFERENCES Member(Id), FOREIGN KEY(StfId) REFERENCES Member(Id), FOREIGN KEY(ItemId) REFERENCES Item(Id));"
